...
export async function unarchiveStaff(staffId: string): Promise<{ error: Error | null }> {
  try {
    ...
    const { error } = await supabase.functions.invoke('manage-archives/unarchive-staff', {
      body: { id: staffId },
      headers: {
...
export async function unarchiveTrainee(traineeId: string): Promise<{ error: Error | null }> {
  try {
    ...
    const { error } = await supabase.functions.invoke('manage-archives/unarchive-trainee', {
      body: { id: traineeId },
      headers: {
        Authorization: `Bearer ${session.access_token}`
      }
