...
export interface AttendanceRequest {
  traineeId?: string;
  staffId?: string;
  status: "absent" | "duty" | "training" | "on_leave" | "return_to_unit" | "suspension" | "resignation" | "termination";
  date: string;
  endDate?: string;
  reason: string;
  leaveType?: string;
...
export async function handleAbsenceRequest(
  ...
  if (existingRecord) {
    // Update existing record - Store reason in the status field with format "status: reason"
    result = await supabaseClient
      .from(tableName)
      .update({ 
    ...
    // Insert new record - Store reason in the status field with format "status: reason"
    result = await supabaseClient
      .from(tableName)
      .insert({
        [personIdField]: personId,
